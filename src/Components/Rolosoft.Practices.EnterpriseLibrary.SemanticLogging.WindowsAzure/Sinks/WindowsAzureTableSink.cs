...
namespace Rolosoft.Practices.EnterpriseLibrary.SemanticLogging.WindowsAzure.Sinks
{
    using System;
    ...
    using SemanticLogging.Properties;
    using SemanticLogging.Sinks;
    ...
    public class WindowsAzureTableSink : IObserver<EventEntry>, IDisposable
    {
        ...
        public WindowsAzureTableSink(string instanceName, string connectionString, string tableAddress, TimeSpan bufferInterval, int maxBufferSize, TimeSpan onCompletedTimeout)
        {
            ...
            {
                throw new ArgumentException(Properties.Resources.InvalidTableName, "tableAddress");
            }
            ...
            table = client.GetTableReference(tableAddress);
            string sinkId = string.Format(CultureInfo.InvariantCulture, "WindowsAzureTableSink ({0})", instanceName);
            bufferedPublisher = BufferedEventPublisher<CloudEventEntry>.CreateAndStart(sinkId, PublishEventsAsync, bufferInterval, BufferCountTrigger, maxBufferSize, cancellationTokenSource.Token);
        }
        ...
        public bool SortKeysAscending
        {
            get
            {
                return sortKeysAscending;
            }

            set
            {
                sortKeysAscending = value;
        ...
        {
            OnNext(value.TryConvertToCloudEventEntry());
        }
        ...
        internal void OnNext(CloudEventEntry value)
        {
            ...
            value.InstanceName = value.InstanceName != null ? NormalizeInstanceName(value.InstanceName) : instanceName;
        ...
        {
            return table.ExecuteBatchAsync(batch, cancellationTokenSource.Token);
        }
        ...
        internal virtual async Task<bool> EnsureTableExistsAsync()
        {
            ...
            try
            {
                await table.CreateIfNotExistsAsync(token).ConfigureAwait(false);
                return true;
            }
        ...
        private List<CloudEventEntry> FilterBatch(IList<CloudEventEntry> collection)
        {
            ...
            foreach (var entry in collection)
            {
                ...
                {
                    entry.CreateKey(SortKeysAscending, salt);
                }
            ...
            foreach (var group in entries.GroupBy(x => x.RowKey))
            {
                ...
                if (items.Count > 1)
                {
                    foreach (var item in items)
                    {
                        ...
                        item.CreateKey(sortKeysAscending, salt);
                    }
                }
            }
        ...
        private async Task<IList<TableResult>> ExecuteBatchSafeAsync(TableBatchOperation batch)
        {
            try
            {
                return await ExecuteBatchAsync(batch).ConfigureAwait(false);
            }
            ...
            if (await EnsureTableExistsAsync().ConfigureAwait(false))
            {
                return await ExecuteBatchAsync(batch).ConfigureAwait(false);
            }
        ...
        {
            var entries = FilterBatch(collection);
            var batchOperation = GetBatchOperation(entries);

            try
            {
                var result = await ExecuteBatchSafeAsync(batchOperation).ConfigureAwait(false);
